...
const properties = [
  {
    ...
    price: 7500000,
    beds: 2, baths: 2, area: 950,
    image: 'https://images.unsplash.com/photo-1600585154526-990dced4db0d?q=80&w=1200&auto=format&fit=crop',
    description: 'Well-ventilated flat with modular kitchen, near metro and schools.'
  },
  {
    id: 'BLR-002',
  ...
  },
  {
    ...
    description: 'Compact apartment with stunning Arabian Sea view, high floor.'
  },
  {
    id: 'PUN-004',
    ...
    <div class="col-sm-6 col-lg-4">
      <div class="card card-property h-100 shadow-sm">
        ...
        <div class="card-body d-flex flex-column">
          ...
          <p class="text-secondary small mb-2"><i class="bi bi-geo-alt"></i> ${p.location}</p>
          <div class="d-flex gap-3 small mb-3">
            <span class="feature"><i class="bi bi-door-open"></i>${p.beds} Beds</span>
            <span class="feature"><i class="bi bi-droplet"></i>${p.baths} Baths</span>
            <span class="feature"><i class="bi bi-aspect-ratio"></i>${p.area} sq.ft</span>
          </div>
          <div class="price mb-3">${inr(p.price)}</div>
          <p class="flex-grow-1 small text-secondary">${p.description}</p>
          <div class="d-grid gap-2 mt-3">
            <button class="btn btn-outline-primary" data-action="details" data-id="${p.id}">
              <i class="bi bi-eye"></i> View Details
            </button>
            <button class="btn btn-success" data-action="whatsapp" data-id="${p.id}">
...
document.querySelectorAll('#cityDropdown .dropdown-menu a')
  .forEach(item => {
    item.addEventListener('click', e => {
      e.preventDefault();
      selectedCity = e.target.dataset.value;
      document.querySelector('#cityDropdown .dropdown-toggle').textContent = e.target.textContent;
      applyFilters();
    });
  });

document.querySelectorAll('#budgetDropdown .dropdown-menu a')
  .forEach(item => {
    item.addEventListener('click', e => {
      e.preventDefault();
      selectedBudget = e.target.dataset.value;
      document.querySelector('#budgetDropdown .dropdown-toggle').textContent = e.target.textContent;
      applyFilters();
    });
  });
...
function applyFilters() {
  const filtered = properties.filter(p => {
    const cityOk = !selectedCity || p.location.toLowerCase().includes(selectedCity);
    let budgetOk = true;
    if (selectedBudget === 'low') budgetOk = p.price < 5000000;
    if (selectedBudget === 'mid') budgetOk = p.price >= 5000000 && p.price <= 10000000;
